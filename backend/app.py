...
async def startup_event():
    ...
    print(f"Files/Folders in Root: {os.listdir('.')}")
    
    # Check for the specific folder structure
    target_folder = "AccidentDetectionProject"
    if os.path.exists(target_folder):
        print(f"SUCCESS: Found '{target_folder}' in root.")
        print(f"Contents of '{target_folder}': {os.listdir(target_folder)}")
        
        models_path = os.path.join(target_folder, "models")
        if os.path.exists(models_path):
            print(f"Contents of 'models': {os.listdir(models_path)}")
    else:
        print(f"ERROR: '{target_folder}' NOT found in root directory.")
        print("Available folders in root:", [d for d in os.listdir('.') if os.path.isdir(d)])
    # =====================================================
